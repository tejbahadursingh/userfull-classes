...
class Format {
	 ...
	 *
	 * @param   mixed  general data to be converted
...
//				$array[$key] = static::to_array($value);
	...
	public function to_xml($data = null, $structure = NULL, $basenode = 'xml')
	{
		...
		if ($structure == NULL)
		{
			$structure = simplexml_load_string("<?xml version='1.0' encoding='utf-8' standalone='yes'?><$basenode />");
		}
		...
		foreach ($data as $key => $value)
		{
			...
			{
                                // checked weather given key is an attribute of node. if yes then add all attribute to node
                                if($key === 'attributes'){
                                    foreach ($value as $attributeName => $attributeValue) {
                                        $node = $structure->addAttribute(htmlspecialchars(html_entity_decode($attributeName, ENT_QUOTES, 'UTF-8'), ENT_QUOTES, "UTF-8"), htmlspecialchars(html_entity_decode($attributeValue, ENT_QUOTES, 'UTF-8'), ENT_QUOTES, "UTF-8"));
                                    }
                                }else {
...
//	}
	...
	{
                $xmlArray = (array) simplexml_load_string($string, 'SimpleXMLElement', true);
